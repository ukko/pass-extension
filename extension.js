...
const getPassword = route => GLib.timeout_add(
...
const PassSearchProvider = new Lang.Class({
    ...
    getInitialResultSet: function(terms, callback, cancellable) {
        let cmd = 'find .password-store -regextype awk -regex "' + terms.map(
            term => '.*' + term + '.*\.gpg').join('|') + '"';
...
const PasswordManager = new Lang.Class({
    ...
    _current_directory: DEFAULT_DIRECTORY,

    _init: function(getPassword) {
        this._getPassword = getPassword;
        PanelMenu.Button.prototype._init.call(this, 0.0);

        let popupMenu = new ScrollablePopupMenu(this.actor, St.Align.START,
            St.Side.TOP);
    ...
    _draw_directory: function() {
        ...
        if (this._current_directory !== DEFAULT_DIRECTORY) {
            let item = new IconMenuItem('go-up-symbolic', this._current_directory);
        ...
        data.sort(function(a, b) {
            ...
            if (element.directory) {
                menuElement = new IconMenuItem('folder-symbolic', element.name + '/');
                ...
                menuElement = new IconMenuItem('dialog-password-symbolic', name);
                menuElement.connect('activate', Lang.bind(this, function() {
